...
export const FlowSheet: React.FC<FlowSheetProps> = ({
  ...
  return (
    <div
      className={`fixed inset-0 ${zIndexClass} flex items-end sm:items-center justify-center print:hidden`}
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      >
        <div className="sm:hidden flex justify-center pt-2 pb-0.5 shrink-0" aria-hidden>
          <span className="h-1 w-10 rounded-full bg-slate-300" />
        </div>

        <header className="flex items-start justify-between gap-3 px-4 pt-2 pb-3 sm:px-5 sm:pt-4 border-b border-slate-100 shrink-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-3 sm:px-5 sm:py-4 custom-scrollbar">
          {children}
        </div>

        {footer ? (
          <footer className="shrink-0 border-t border-slate-100 bg-white/95 backdrop-blur px-4 py-3 sm:px-5 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
            {footer}
          </footer>
        ) : null}
      </div>
