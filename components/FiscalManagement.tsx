    ...
    <div className="space-y-6 animate-in fade-in duration-300">
                      ...
                      <tr key={order.id} className="border-t border-slate-100">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-[10px] font-black uppercase text-slate-500">
                            {order.nfeTipo === 'devolucao' ? 'Devolução' : order.nfeTipo === 'transferencia' ? 'Transferência' : 'Venda'}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg ${
                            order.nfeStatus === 'autorizada' ? 'bg-emerald-50 text-emerald-800' :
                            order.nfeStatus === 'cancelada' ? 'bg-rose-50 text-rose-700' :
                            order.nfeStatus === 'rejeitada' ? 'bg-amber-50 text-amber-800' :
                            'bg-slate-100 text-slate-600'
                          }`}>
                            {order.nfeStatus || 'nao_emitida'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right font-black">{formatBRL(order.total)}</td>
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap items-center justify-center gap-1.5">
                            <button onClick={() => setSelectedDanfeOrder(order)} className="px-2.5 py-1.5 bg-slate-900 text-white rounded-xl text-[10px] font-black inline-flex items-center gap-1">
                              <Eye size={12} /> DANFE
                            </button>
                            {order.nfeStatus === 'autorizada' && (
                              <>
                                <button
                                  ...
                                  className="px-2.5 py-1.5 bg-amber-50 text-amber-800 border border-amber-200 rounded-xl text-[10px] font-black inline-flex items-center gap-1"
                                >
                                  <Undo2 size={12} /> Devolver
                                </button>
                                <button
                                  onClick={() => setOrderToCancel(order)}
                                  className="px-2.5 py-1.5 bg-rose-50 text-rose-700 border border-rose-200 rounded-xl text-[10px] font-black inline-flex items-center gap-1"
                                >
                ...
                <div key={order.id} className="p-5 bg-slate-50 border border-slate-200 rounded-3xl space-y-3">
                  <div className="flex justify-between gap-3 flex-wrap">
                    ...
                    <div className="text-right">
                      ...
                      <button
                        onClick={() => { setIsTransferenciaEmit(false); setDevolutionChave(undefined); setOrderToEmitNfe(order); }}
                        className="mt-2 px-4 py-2 bg-purple-600 text-white rounded-2xl text-xs font-black"
                      >
                        Emitir NF-e
                      </button>
                      <button
                        onClick={() => { setIsTransferenciaEmit(true); setDevolutionChave(undefined); setOrderToEmitNfe(order); }}
                        className="mt-2 ml-2 px-3 py-2 bg-sky-50 text-sky-800 border border-sky-200 rounded-2xl text-xs font-black inline-flex items-center gap-1"
                      >
                  ...
                  <div className="flex flex-wrap gap-2 text-[11px]">
                    {(order.items || []).map((it, idx) => (
                      <span key={idx} className="bg-white px-2.5 py-1 rounded-lg border font-bold">
                        {it.productName}: {it.quantity} {it.unit || 'Ton'} · CFOP {it.cfop || '—'} · CST {it.cst || it.csosn || '—'}
                      </span>
                    ))}
                  </div>
                </div>
      ...
      {orderToEmitNfe && (
        <EmitirNfeModal
          ...
          onClose={() => { setOrderToEmitNfe(null); setIsTransferenciaEmit(false); setDevolutionChave(undefined); }}
